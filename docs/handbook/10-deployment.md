...
Select a published image through the [release workflow](09-release.md). The application's target environments, promotion
procedure, secret delivery, and deployment ownership remain [documented gaps](02-project.md#documentation-coverage).
...
automatically. Resource paths and backend settings are documented in [configuration](04-configuration.md#configuration-sources).
...
After [tool setup](05-development.md#environment-setup), validate the chart from the repository root:
...
Keep migration naming and authoring rules in [conventions](03-conventions.md#architecture-and-data). Existing
...
Run the relevant engine cases with the [test environment](06-testing.md#integration-test-environment) configured.
...
[message upgrade guidance](04-configuration.md#nats-message-format).
...
Confirm startup and migration completion in logs, then verify [liveness and readiness](11-operations.md#run-and-verify-locally)
...
application uses events, verify [persisted delivery progress](11-operations.md#inspect-outbox-delivery); readiness alone
does not establish broker or consumer health. Check [monitoring](12-monitoring.md) for traffic, errors, and dependency
signals.

If rollout fails, capture the running image, migration state, and error evidence before another change. Follow
[failure investigation and recovery](11-operations.md#failure-investigation-and-recovery) and the change's migration handoff
